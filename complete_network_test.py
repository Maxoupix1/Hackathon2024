...
training_data = pd.read_csv('training_data_second.csv').sample(frac=0.1, random_state=42)  # Sample 10% for quick testing
...
# Entraîner le modèle
model.train()
for epoch in range(5):  # Fewer epochs for quick testing
...
prediction_data = pd.read_csv('prediction_data.csv').sample(frac=0.1, random_state=42)  # Sample 10% for quick testing
...
with torch.no_grad():
    ...
    predictions = model(X_pred_tensor)  # Add sequence dimension here if needed
...
output.to_csv('output_nn_test.csv', index=False)
print('Les prédictions ont été réalisées et le fichier output_nn_test.csv a été sauvegardé avec succès.')
